...
export type ReportTagPeriodSummary = {
  tagId: string;
  tagName?: string;
  unit?: string | null;
  count?: number;
...
export type ReportBillingFormulaContext = {
  totalKwh?: number;
  energyCost?: number;
  demandCost?: number;
  grandTotal?: number;
...
export function isEnergyLikeTag(unit?: string | null, tagName?: string | null): boolean {
...
export function resolveReportDateRange(input: {
  reportDefaultRange?: string | null;
  reportType?: string | null;
  objectPeriod?: string | null;
...
export function tagSummaryFromHistoryRows(
  tagId: string,
  rows: Array<{ value: number | null; readAt?: Date | string }>,
  meta?: { tagName?: string; unit?: string | null },
): ReportTagPeriodSummary {
  const numeric = rows.filter((r) => typeof r.value === 'number' && Number.isFinite(r.value));
  const values = numeric.map((r) => r.value as number);
...
export function enrichTagSummariesWithBilling(
  summaries: ReportTagPeriodSummary[],
...
function replaceToken(expr: string, token: string, value: number | null): string {
  if (value == null || !Number.isFinite(value)) return expr;
  return expr.replaceAll(token, String(value));
}

function replaceLetterMetric(
  expr: string,
  letter: string,
  metric: TagMetric,
  tagIds: string[],
  summaryById: Map<string, ReportTagPeriodSummary>,
): string {
...
export function evaluateReportFormulaExpression(
  formula: string,
  tagIds: string[],
  tagSummaries: ReportTagPeriodSummary[],
  billing?: ReportBillingFormulaContext | null,
): number | null {
...
export function formatReportFormulaResult(value: number | null, decimalPlaces = 2): string {
...
export function resolveFieldMetricValue(
  metric: ReportFieldMetric,
  summary: ReportTagPeriodSummary | undefined,
...
export function billingToFormulaContext(
  billing: ReportBillingFormulaContext & Record<string, unknown>,
): ReportBillingFormulaContext {
  return {
