...
export function validateCarbonConfig(input) {
    const issues = [];
    ...
    const carbonDeviceIds = new Set(input.devices
        .filter(d => {
        const m = resolveDeviceEnergyMapping(d);
        if (hasMain)
            return m.role === 'site_main' && m.includeInCarbon;
    ...
    const unmappedKwh = input.tags.filter(t => {
        if (normalizeTagEnergyRole(t.energyTagRole) !== 'none')
            return false;
    ...
    if (input.tagQuality && carbonTags.length > 0) {
        ...
        const allBad = energyTags.length > 0 &&
            energyTags.every(t => {
...
export function suggestPrimaryImportTagName(tags) {
    const ranked = tags
