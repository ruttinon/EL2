...
import { isEnergyLikeTag, type ReportBillingFormulaContext, type ReportTagPeriodSummary } from './reportFormula.js';

export type MeterBillingColumnId =
  | 'index'
  | 'device'
  | 'tag'
  | 'meterNo'
...
export const METER_BILLING_COLUMN_DEFS: Array<{ id: MeterBillingColumnId; label: string; labelTh: string }> = [
...
export const DEFAULT_METER_BILLING_COLUMNS: MeterBillingColumnId[] = [
  'index', 'device', 'tag', 'meterNo', 'first', 'last', 'usage', 'rate', 'amount',
];

export type MeterBillingRow = {
  index: number;
  tagId: string;
  tagName: string;
  deviceId: string;
...
export type MeterBillingTagInput = {
  id: string;
  name: string;
  deviceId: string;
  unit?: string | null;
...
export type MeterBillingDeviceInput = {
  id: string;
  name: string;
  type?: string | null;
...
export type ReportDataScopeMode = 'project' | 'device' | 'converter';

export type MeterBillingTableProps = {
  scopeMode?: ReportDataScopeMode;
  scopeDeviceId?: string;
  deviceId?: string;
  deviceIds?: string[];
  tagIds?: string[];
  /** When true (default), include all import/net kWh tags */
  autoInclude?: boolean;
  columns?: string;
  showHeader?: boolean;
  period?: string;
...
export function resolveReportScopeDeviceIds(
  devices: MeterBillingDeviceInput[],
  props?: MeterBillingTableProps | null,
): string[] {
  ...
  if (props?.deviceIds?.length) props.deviceIds.forEach((id) => ids.add(id));
  if (scopeDeviceId) ids.add(scopeDeviceId);
...
export function isBillingEnergyTag(tag: MeterBillingTagInput): boolean {
  const role = normalizeTagEnergyRole(tag.energyTagRole);
  if (role === 'import_kwh' || role === 'net_kwh') return true;
...
export function parseMeterBillingColumns(raw?: string | null): MeterBillingColumnId[] {
  const allowed = new Set(METER_BILLING_COLUMN_DEFS.map((c) => c.id));
  if (!raw?.trim()) return [...DEFAULT_METER_BILLING_COLUMNS];
  const cols = raw.split(',').map((s) => s.trim() as MeterBillingColumnId);
...
export function listMeterBillingTags(
  tags: MeterBillingTagInput[],
  devices: MeterBillingDeviceInput[],
  props?: MeterBillingTableProps | null,
): MeterBillingTagInput[] {
...
export function meterNumberForTag(tag: MeterBillingTagInput): string {
  const desc = String(tag.description ?? '').trim();
  if (desc && /^\d{6,}$/.test(desc.replace(/\s/g, ''))) return desc.trim();
  return tag.name;
}

export function buildMeterBillingRows(
  tags: MeterBillingTagInput[],
  devices: MeterBillingDeviceInput[],
  summaries: Map<string, ReportTagPeriodSummary> | Record<string, ReportTagPeriodSummary>,
  props?: MeterBillingTableProps | null,
...
export function formatMeterCell(
  column: MeterBillingColumnId,
  row: MeterBillingRow,
  decimalPlaces = 2,
): string {
...
export function meterBillingColumnLabel(column: MeterBillingColumnId, lang: 'en' | 'th' = 'th'): string {
  const def = METER_BILLING_COLUMN_DEFS.find((c) => c.id === column);
  if (!def) return column;
