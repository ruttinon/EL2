...
export function parsePorts(raw: unknown): GraphicPort[] {
  ...
  for (const part of raw.split(';')) {
    ...
    const kind: PortKind | undefined =
      portId.startsWith('in') ? 'in' : portId.startsWith('out') ? 'out' : 'bidirectional';
    ports.push({
      id: portId,
...
export function formatPorts(ports: GraphicPort[]): string {
  return ports
    .map((p) => {
