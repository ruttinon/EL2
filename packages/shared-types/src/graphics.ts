...
export const GRAPHIC_LAYOUT_VERSION = 1 as const;
export const GRAPHIC_LAYOUT_VERSION_V2 = 2 as const;
export const GRAPHIC_LAYOUT_VERSION_V3 = 3 as const;
export const GRAPHIC_PACKAGE_VERSION = 1 as const;

export type GraphicLayoutVersion =
  | typeof GRAPHIC_LAYOUT_VERSION
  | typeof GRAPHIC_LAYOUT_VERSION_V2
  | typeof GRAPHIC_LAYOUT_VERSION_V3;

export type { GraphicObjectTransform } from './layoutV3.js';

export type GraphicObjectType =
  // Existing tools
  | 'text' | 'image' | 'value' | 'gauge' | 'trend' | 'alarm' | 'line' | 'rectangle' | 'button' | 'circle' | 'polygon' 
  | 'switch' | 'slider' | 'led' | 'status' | 'levelbar' | 'multistate' | 'navbutton' | 'tagtable' | 'alarmtable' 
  | 'sparkline' | 'barchart' | 'panel' | 'hotspot' | 'tabbar' | 'flowpath' | 'elecsymbol' | 'sprite' | 'lottie' 
  | 'viewport3d' | 'scene3d' | 'group' | 'kpicard' | 'piechart' | 'formulavalue' | 'statusbadge' | 'zone3d' | 'cable3d' 
  | 'bussection' | 'feedlabel' | 'zone2d' | 'wall' | 'echart' | 'ellipse' | 'inputfield' | 'dropdown' | 'iframe' 
  | 'video' | 'progressbar' | 'semaphore' | 'pipe' | 'clock'
  // New Report Editor tools
  | 'section' | 'headerfooter' | 'pagebreak' 
  | 'qrcode' | 'signature' 
  | 'variable' 
  | 'toutable' 
...
export function isHtmlGraphicPage(layout: GraphicLayout | null | undefined): boolean {
  return layout?.pageKind === 'html';
}

export function isCanvasGraphicPage(layout: GraphicLayout | null | undefined): boolean {
  return !layout?.pageKind || layout.pageKind === 'canvas';
}

/** Canvas page dominated by a full-size scene3d GLB (building digital twin). */
export function isGlbBuildingGraphic(
  layout: GraphicLayout | null | undefined,
  pageWidth = 1366,
  pageHeight = 768,
): boolean {
  if (!layout || isHtmlGraphicPage(layout)) return false;
  const scene = (layout.objects ?? []).find(
