...
import type { CreateDeviceInput, DeviceDatabaseStatus, DeviceSummary, DeviceTreeNode, DeviceType, UpdateDeviceInput } from '@energylink/shared-types';
...
async function collectDeviceAndDescendantIds(client: PrismaClient, rootId: string) {
...
async function deleteDeviceGraph(client: PrismaClient, rootId: string) {
  ...
  if (!root) throw new Error('Device not found.');
  ...
  await client.$transaction(async tx => {
    ...
    if (alarmIds.length) await tx.alarmNotificationEvent.deleteMany({ where: { alarmId: { in: alarmIds } } });
    await tx.historyValue.deleteMany({ where: { deviceId: { in: deviceIds } } });
    if (tagIds.length) await tx.historyValue.deleteMany({ where: { tagId: { in: tagIds } } });
    if (alarmIds.length) await tx.alarm.deleteMany({ where: { id: { in: alarmIds } } });
...
function toDeviceSummary(device: any): DeviceSummary {
  return {
    id: device.id,
...
export async function listDevices(projectId?: string): Promise<DeviceSummary[]> {
  const client = getClient();
...
export async function getDeviceTree(projectId?: string): Promise<DeviceTreeNode[]> {
  const devices = await listDevices(projectId);
  const byId = new Map<string, DeviceTreeNode>();
  for (const d of devices) byId.set(d.id, { ...d, children: [] });
  const roots: DeviceTreeNode[] = [];
  for (const node of byId.values()) {
    if (node.parentDeviceId && byId.has(node.parentDeviceId)) byId.get(node.parentDeviceId)!.children.push(node);
    else roots.push(node);
  }
  return roots;
}

function assertDeviceInput(input: CreateDeviceInput) {
  if (!input.name?.trim()) throw new Error('Device Name is required.');
  if (!input.type) throw new Error('Device Type is required.');
  if (input.type === 'converter' && input.parentDeviceId) throw new Error('Converter must be a first-level device only.');
  if ((input.type === 'meter' || input.type === 'sensor') && !input.parentDeviceId) throw new Error('Meter/Sensor must use a parent converter.');
}

function normalizeDeviceProtocol(protocol?: string | null) {
  if (protocol === 'tcp' || protocol === 'udp' || protocol === 'modbus_tcp' || protocol === 'modbus_rtu' || protocol === 'cvm_c4' || protocol === 'cvm_c11' || protocol === 'xgmb_meter') return protocol;
  return 'modbus_tcp';
}

export async function createDevice(input: CreateDeviceInput): Promise<DeviceSummary> {
  ...
  if (input.parentDeviceId) {
    ...
    if (!parent) throw new Error('Parent converter was not found in the current project.');
    if (parent.type !== 'converter') throw new Error('Parent device must be a converter.');
  }
...
export async function updateDevice(input: UpdateDeviceInput): Promise<DeviceSummary> {
  const client = getClient();
  ...
  if ('ipAddress' in data && typeof data.ipAddress === 'string') data.ipAddress = data.ipAddress.trim() || null;
  if ('serialPort' in data && typeof data.serialPort === 'string') data.serialPort = data.serialPort.trim() || null;
  if ('imageDataUrl' in data) data.imageDataUrl = typeof data.imageDataUrl === 'string' ? data.imageDataUrl.trim() || null : null;
  if ('model3dUrl' in data) data.model3dUrl = typeof data.model3dUrl === 'string' ? data.model3dUrl.trim() || null : null;
  ...
  if ('parity' in data && typeof data.parity === 'string') data.parity = ['none', 'even', 'odd'].includes(data.parity) ? data.parity : 'none';
...
export async function getDeviceDatabaseStatus(projectId?: string): Promise<DeviceDatabaseStatus> {
  const client = getClient();
  const activeProjectId = await getActiveProjectId(projectId);
  const [deviceCount, converterCount, meterCount, sensorCount] = await Promise.all([
    ...
    client.device.count({ where: { projectId: activeProjectId, type: 'converter' as DeviceType } }),
    client.device.count({ where: { projectId: activeProjectId, type: 'meter' as DeviceType } }),
    client.device.count({ where: { projectId: activeProjectId, type: 'sensor' as DeviceType } })
  ]);
