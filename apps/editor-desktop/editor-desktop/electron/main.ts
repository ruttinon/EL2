...
function registerReportIpc() {
  ipcMain.handle('reports:list', (_event, projectId?: string) => listReports(projectId));
  ipcMain.handle('reports:get', (_event, id: string) => getReport(id));
  ...
  ipcMain.handle('reports:delete', (_event, id: string) => deleteReport(id));
  ipcMain.handle('reports:status', (_event, projectId?: string) => getReportDatabaseStatus(projectId));
}

function registerGraphicIpc() {
  ipcMain.handle('graphics:list', (_event, projectId?: string) => listGraphics(projectId));
  ipcMain.handle('graphics:get', (_event, id: string) => getGraphic(id));
  ...
  ipcMain.handle('graphics:delete', (_event, id: string) => deleteGraphic(id));
  ipcMain.handle('graphics:status', (_event, projectId?: string) => getGraphicDatabaseStatus(projectId));
}

function registerDeviceIpc() {
  ipcMain.handle('devices:list', (_event, projectId?: string) => listDevices(projectId));
  ipcMain.handle('devices:tree', (_event, projectId?: string) => getDeviceTree(projectId));
  ...
  ipcMain.handle('devices:delete', (_event, id: string) => deleteDevice(id));
  ipcMain.handle('devices:status', (_event, projectId?: string) => getDeviceDatabaseStatus(projectId));
}

function registerTagIpc() {
  ipcMain.handle('tags:list', (_event, projectId?: string) => listTags(projectId));
  ipcMain.handle('tags:listByDevice', (_event, deviceId: string) => listTagsByDevice(deviceId));
  ipcMain.handle('tags:create', (_event, input) => createTag(input));
  ipcMain.handle('tags:update', (_event, input) => updateTag(input));
  ipcMain.handle('tags:delete', (_event, id: string) => deleteTag(id));
  ipcMain.handle('tags:status', (_event, projectId?: string) => getTagDatabaseStatus(projectId));
}
...
function registerProjectIpc() {
  ...
  ipcMain.handle('projects:setActive', (_event, id: string) => setActiveProject(id));
  ipcMain.handle('projects:status', () => getProjectDatabaseStatus());
}

function registerUtilsIpc() {
  ipcMain.handle('utils:openFile', async (_event, filters: { name: string; extensions: string[] }[]) => {
