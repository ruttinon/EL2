...
export function computeDeviceQualifiedKwh(deviceTags, options) {
...
export function selectCarbonDeviceIds(devices) {
    ...
    const included = resolved.filter(d => d.mapping.includeInCarbon &&
...
export function computeCarbonSummary(input) {
    const emissionFactorKgPerKwh = Number.isFinite(Number(input.emissionFactorKgPerKwh)) && Number(input.emissionFactorKgPerKwh) >= 0
