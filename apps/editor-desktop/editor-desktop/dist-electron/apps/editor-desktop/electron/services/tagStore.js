...
function assertTagInput(input) {
    if (!input.deviceId)
        throw new Error('Select a device for the tag.');
    if (!input.name?.trim())
        throw new Error('Tag Name is required.');
    if (!Number.isInteger(input.address) || input.address <= 0)
        throw new Error('Address must be an integer greater than 0.');
    if (input.decimalPlaces !== undefined && (input.decimalPlaces < 0 || input.decimalPlaces > 8))
...
export async function listTagsByDevice(deviceId) {
    const client = getClient();
    const tags = await client.tag.findMany({
        where: { deviceId },
        include: { device: true },
...
export async function createTag(input) {
    ...
    if (!device)
        throw new Error('Device was not found in the current project.');
...
export async function updateTag(input) {
    ...
    if ('alarmHigh' in data && data.alarmHigh === undefined)
        delete data.alarmHigh;
    if ('alarmLow' in data && data.alarmLow === undefined)
        delete data.alarmLow;
...
export async function getTagDatabaseStatus(projectId) {
    const client = getClient();
    const activeProjectId = await getActiveProjectId(projectId);
