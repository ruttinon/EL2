...
async function openClient(device: RuntimeDevice): Promise<OpenedClient> {
    ...
    if (transportProtocol === 'modbus_tcp') {
        ...
        if (!resolved.profile) return { error: resolved.error ?? 'Invalid CVM-C4 Modbus TCP configuration.' };
    ...
    if (transportProtocol === 'tcp') {
        const resolved = resolveTcpProfile(device);
        if (!resolved.profile) return { error: resolved.error ?? 'Invalid CVM-C4 RTU-over-TCP tunnel configuration.' };
        client.setTimeout(resolved.profile.timeoutMs);
        closeClient(client);
    ...
    if (transportProtocol === 'udp') {
        const resolved = resolveTcpProfile(device);
        if (!resolved.profile) return { error: resolved.error ?? 'Invalid CVM-C4 RTU-over-UDP tunnel configuration.' };
        client.setTimeout(resolved.profile.timeoutMs);
        closeClient(client);
    ...
    if (transportProtocol === 'modbus_rtu') {
        ...
        if (!resolved.profile) return { error: resolved.error ?? 'Invalid CVM-C4 Modbus RTU configuration.' };
        client.setTimeout(resolved.profile.timeoutMs);
    ...
    closeClient(client);
    return { error: `Unsupported converter protocol for CVM-C4: ${transportProtocol || 'unknown'}` };
}

export class CvmC4Driver implements DeviceDriver {
    readonly protocol = 'cvm_c4';
    ...
    async testConnection(device: RuntimeDevice): Promise<DeviceConnectionResult> {
        ...
        try {
            ...
            if (!opened.client) {
                return finishConnectionResult(device, startedAt, false, 'invalid_configuration', opened.error ?? 'Invalid CVM-C4 configuration.', opened.error, opened.endpoint);
            }
            client = opened.client;
            return finishConnectionResult(device, startedAt, true, 'online', 'CVM-C4 connection opened successfully.', undefined, opened.endpoint);
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            return finishConnectionResult(device, startedAt, false, 'offline', 'CVM-C4 connection failed.', message);
        } finally {
        ...
        try {
            ...
            if (!opened.client) {
                return {
                    ...
                    readAt: new Date().toISOString(),
                    elapsedMs: 0,
                    message: opened.error ?? 'Invalid CVM-C4 configuration.',
                    error: opened.error,
            ...
            return {
                ...
                message: 'Tag was read from the CVM-C4 meter successfully.',
                endpoint: opened.endpoint
            };
        } catch (error) {
            const completedAt = new Date();
            ...
            return {
                ...
                readAt: completedAt.toISOString(),
                elapsedMs: completedAt.getTime() - startedAt.getTime(),
                message: 'CVM-C4 tag read failed.',
                error: message
            };
        } finally {
            if (client) {
    ...
    async readTags(device: RuntimeDevice, tags: Tag[]): Promise<ReadTagOnceResult[]> {
        ...
        try {
            ...
            return tags.map((tag) => ({
                ...
                readAt: completedAt,
                elapsedMs: 0,
                message: 'CVM-C4 grouped read failed.',
                error: message
            }));
        } finally {
            if (client) {
    ...
    async writeTag(device: RuntimeDevice, tag: Tag, value: number | boolean): Promise<WriteTagResult> {
        ...
        try {
            ...
            return {
                ...
                elapsedMs: completedAt.getTime() - startedAt.getTime(),
                message: 'Tag value written to CVM-C4 meter successfully.',
                endpoint: opened.endpoint
            };
        } catch (error) {
            const completedAt = new Date();
            ...
            return {
                ...
                writtenAt: completedAt.toISOString(),
                elapsedMs: completedAt.getTime() - startedAt.getTime(),
                message: 'CVM-C4 tag write failed.',
                error: message
            };
        } finally {
            if (client) {
