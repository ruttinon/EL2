...
async function openClient(device: RuntimeDevice): Promise<OpenedClient> {
    ...
    if (transportProtocol === 'modbus_tcp') {
        ...
        if (!resolved.profile) return { error: resolved.error ?? 'Invalid Imported XGMB meter Modbus TCP configuration.' };
    ...
    if (transportProtocol === 'tcp') {
        const resolved = resolveTcpProfile(device);
        if (!resolved.profile) return { error: resolved.error ?? 'Invalid Imported XGMB meter RTU-over-TCP tunnel configuration.' };
        client.setTimeout(resolved.profile.timeoutMs);
        closeClient(client);
    ...
    if (transportProtocol === 'udp') {
        const resolved = resolveTcpProfile(device);
        if (!resolved.profile) return { error: resolved.error ?? 'Invalid Imported XGMB meter RTU-over-UDP tunnel configuration.' };
        client.setTimeout(resolved.profile.timeoutMs);
        closeClient(client);
    ...
    if (transportProtocol === 'modbus_rtu') {
        ...
        if (!resolved.profile) return { error: resolved.error ?? 'Invalid Imported XGMB meter Modbus RTU configuration.' };
        client.setTimeout(resolved.profile.timeoutMs);
    ...
    closeClient(client);
    return { error: `Unsupported converter protocol for Imported XGMB meter: ${transportProtocol || 'unknown'}` };
}

export class XgmbMeterDriver implements DeviceDriver {
    readonly protocol = 'xgmb_meter';
    ...
    async testConnection(device: RuntimeDevice): Promise<DeviceConnectionResult> {
        ...
        try {
            ...
            if (!opened.client) {
                return finishConnectionResult(device, startedAt, false, 'invalid_configuration', opened.error ?? 'Invalid Imported XGMB meter configuration.', opened.error, opened.endpoint);
            }
            client = opened.client;
            return finishConnectionResult(device, startedAt, true, 'online', 'Imported XGMB meter connection opened successfully.', undefined, opened.endpoint);
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            return finishConnectionResult(device, startedAt, false, 'offline', 'Imported XGMB meter connection failed.', message);
        } finally {
        ...
        try {
            ...
            if (!opened.client) {
                return {
                    ...
                    readAt: new Date().toISOString(),
                    elapsedMs: 0,
                    message: opened.error ?? 'Invalid Imported XGMB meter configuration.',
                    error: opened.error,
            ...
            return {
                ...
                message: 'Tag was read from the Imported XGMB meter meter successfully.',
                endpoint: opened.endpoint
            };
        } catch (error) {
            const completedAt = new Date();
            ...
            return {
                ...
                readAt: completedAt.toISOString(),
                elapsedMs: completedAt.getTime() - startedAt.getTime(),
                message: 'Imported XGMB meter tag read failed.',
                error: message
            };
        } finally {
            if (client) {
    ...
    async readTags(device: RuntimeDevice, tags: Tag[]): Promise<ReadTagOnceResult[]> {
        ...
        try {
            ...
            return tags.map((tag) => ({
                ...
                readAt: completedAt,
                elapsedMs: 0,
                message: 'Imported XGMB meter grouped read failed.',
                error: message
            }));
        } finally {
            if (client) {
    ...
    async writeTag(device: RuntimeDevice, tag: Tag, value: number | boolean): Promise<WriteTagResult> {
        ...
        try {
            ...
            return {
                ...
                elapsedMs: completedAt.getTime() - startedAt.getTime(),
                message: 'Tag value written to Imported XGMB meter meter successfully.',
                endpoint: opened.endpoint
            };
        } catch (error) {
            const completedAt = new Date();
            ...
            return {
                ...
                writtenAt: completedAt.toISOString(),
                elapsedMs: completedAt.getTime() - startedAt.getTime(),
                message: 'Imported XGMB meter tag write failed.',
                error: message
            };
        } finally {
            if (client) {
