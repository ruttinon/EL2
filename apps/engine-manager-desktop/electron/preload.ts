...
contextBridge.exposeInMainWorld('engineManagerApi', {
  ...
  readRecentLogLines: (limit?: number) => ipcRenderer.invoke('engine-manager:readRecentLogLines', limit),
  readRuntimeSettings: () => ipcRenderer.invoke('engine-manager:readRuntimeSettings'),
  saveRuntimeSettings: (config: Record<string, unknown>) => ipcRenderer.invoke('engine-manager:saveRuntimeSettings', config)
});
