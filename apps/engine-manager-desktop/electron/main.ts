...
let localEngineProcess: ChildProcessWithoutNullStreams | null = null;
...
function normalizeEngineConfig(input: Partial<EngineConfig>): EngineConfig {
...
function validateEngineConfig(config: EngineConfig) {
  ...
  if (!config.engineName.trim()) errors.push('Engine name is required.');
  if (!config.apiHost.trim()) errors.push('API host is required.');
  if (!Number.isInteger(config.port) || config.port < 1 || config.port > 65535) errors.push('API port must be an integer between 1 and 65535.');
  if (!Number.isInteger(config.pollingScanIntervalMs) || config.pollingScanIntervalMs < 250) errors.push('Polling interval must be at least 250 ms.');
  if (!Number.isInteger(config.historyRetentionDays) || config.historyRetentionDays < 1) errors.push('History retention must be at least 1 day.');
  if (!Number.isInteger(config.logRetentionDays) || config.logRetentionDays < 1) errors.push('Log retention must be at least 1 day.');
  if (!Number.isInteger(config.backupRetentionDays) || config.backupRetentionDays < 1) errors.push('Backup retention must be at least 1 day.');
  const pathFields: Array<[keyof EngineConfig, string]> = [
  ...
  for (const [field, label] of pathFields) {
    if (!String(config[field] ?? '').trim()) errors.push(`${label} is required.`);
  }
  if (config.port === 80) warnings.push('Port 80 may require administrator privileges on Windows.');
  if (config.apiHost === '0.0.0.0' && !config.requireAuthentication) warnings.push('Remote access should be enabled only on trusted local networks.');
  if (!config.historyLoggingEnabled) warnings.push('History logging is disabled. Trend and report data may not be available.');
  if (config.runtimeWriteMode === 'read_only') warnings.push('Runtime write mode is read-only. Polling will not write current values or history.');
...
async function readEngineConfig() {
  try {
    ...
    const parsed = JSON.parse(raw.replace(/^\uFEFF/, '')) as Partial<EngineConfig>;
...
async function writeEngineConfig(config: EngineConfig) {
  const normalized = normalizeEngineConfig(config);
  const validation = validateEngineConfig(normalized);
  if (!validation.valid) throw new Error(`Invalid engine configuration: ${validation.errors.join(' ')}`);
...
async function appendLocalEngineLog(text: string) {
...
async function saveRuntimeSettings(incoming: Partial<EngineConfig>) {
  ...
  const changedKeys = Object.keys(next).filter((key) => {
    const typedKey = key as keyof EngineConfig;
...
function registerIpc() {
  ...
  ipcMain.handle('engine-manager:saveRuntimeSettings', (_event, incoming: Partial<EngineConfig>) => saveRuntimeSettings(incoming));
}
